...
own cannot be decrypted. Lose the key and every credential has to be added
again.
</Callout>
...
A keychain belongs to one machine and one signed-in user. Firetower hands
credentials to workers on other machines and to containers with no desktop
session.
